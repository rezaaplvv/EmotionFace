...
const ParticleOverlay = ({ type }) => {
  ...
  for (let i = 0; i < count; i++) {
    ...
    particles.push(
      <div key={i} className="absolute top-[-10%] animate-fall select-none"
        style={{ left, animationDelay: delay, animationDuration: duration, fontSize: size, opacity: 0.6 }}>
        {randomEmoji}
      </div>
    );
  }
...
export default function Home() {
  ...
  const creativeData = {
    happy: { playlistUrl: "https://spotify.com", playlistLabel: "Putar Lagu Happy 🎵" },
    sad: { playlistUrl: "https://spotify.com", playlistLabel: "Temani Aku Galau 🥀" },
    angry: { playlistUrl: "https://spotify.com", playlistLabel: "Lepaskan Emosi 🎸" },
    surprised: { playlistUrl: "https://spotify.com", playlistLabel: "Party Vibes 🎉" },
    neutral: { playlistUrl: "https://spotify.com", playlistLabel: "Chill Mode ☕" }
  };
    ...
    <main className={`h-screen w-full overflow-hidden relative flex flex-col transition-all duration-1000 ${currentVibe.pageBg} text-white`}>
      ...
      <div className={`flex-1 flex flex-col items-center justify-center gap-4 md:gap-8 px-4 relative z-10 ${isAngry ? 'animate-vibe-shake' : ''}`}>
        <div className="text-center space-y-2 md:space-y-4">
          <h2 className="text-2xl md:text-4xl font-black tracking-tighter uppercase italic">Detect Your <span className={currentVibe.textInfo.color}>Vibe</span></h2>
          <div className="flex flex-col items-center gap-2">
            <div className={`px-4 py-1 rounded-full border border-white/10 bg-black/40 backdrop-blur-sm`}>
               <span className={`text-xs md:text-sm font-bold uppercase ${currentVibe.textInfo.color}`}>{emotion}</span>
            </div>
            <div className="h-6 flex items-center justify-center px-4">
              <p className={`text-sm md:text-lg font-medium italic opacity-80 text-center ${currentVibe.textInfo.color}`}>"{randomQuote}"</p>
            </div>
          </div>
        </div>
